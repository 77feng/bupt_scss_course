...
class WaterMark:
    ...
    def extract(self, filename=None, embed_img=None, wm_shape=None, out_wm_name=None, mode='img'):
        ...
        if mode == 'img':
            ...
            _, binary_image = cv2.threshold(wm, 128, 1, cv2.THRESH_BINARY)

            # 保存二值图
            binary_image_save = (binary_image * 255).astype(np.uint8)  # 转回可视化的0~255范围
            cv2.imwrite(out_wm_name, binary_image_save)
        elif mode == 'str':
