...
def embed_message(original_image, binary_message):
    ...
    message_index = 0
    for col in range(original_image.shape[1]):  # 遍历列
        for row in range(original_image.shape[0]):  # 遍历行
            if message_index < len(binary_message):
                # 将最低位替换为秘密信息的位
                if binary_message[message_index] == '0':
                    stego_image[row, col] = stego_image[row, col] & 0xFE  # 清除最低有效位
                else:
                    stego_image[row, col] = stego_image[row, col] | 0x01  # 设置最低有效位为1
                message_index += 1
...
def extract_lsb(image):
    """逐列提取最低有效位"""
    bits = []
    for col in range(image.shape[1]):  # 遍历列
        for row in range(image.shape[0]):  # 遍历行
            # 提取最低有效位
            lsb = image[row, col] & 0x01
            bits.append(str(lsb))

    result = ''.join(bits)
    print(f"\n本人学号为{student_id}，本人姓名为{name}，隐写图像的最低有效位为：{result}")
    return result
