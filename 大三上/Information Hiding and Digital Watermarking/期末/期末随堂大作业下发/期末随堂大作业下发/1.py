import numpy as np
import matplotlib.pyplot as plt
import os

student_id = "2022211570"
name = "项枫"

# 设置终端颜色为白底黑字
print('\033[47m\033[30m', end='')  # 47:白色背景, 30:黑色文字


def get_binary_message(secret_message="BH"):
    """获取秘密信息的ASCII和二进制表示"""
    binary_message = []
    ascii_values = []

    for char in secret_message:
        ascii_val = ord(char)
        ascii_values.append(ascii_val)
        binary = format(ascii_val, '08b')
        binary_message.append(binary)

    print(f"本人学号为{student_id}，本人姓名为{name}，秘密信息的ASCII是：{ascii_values}")
    print("转成的二进制是：")
    for binary in binary_message:
        print(binary)

    return ''.join(binary_message)


def plot_histogram(image, title_prefix="原始"):
    """绘制灰度值直方图"""
    plt.rcParams['font.sans-serif'] = ['SimHei']
    plt.rcParams['axes.unicode_minus'] = False
    plt.figure(figsize=(10, 6))

    # 创建整数刻度
    bins = list(range(60, 81))  # 注意：bins 的上限需要比最大值大1

    n, bins, patches = plt.hist(image.flatten(), bins=bins, edgecolor='black')

    # plt.hist(image.flatten(), bins=bins, edgecolor='black')
    plt.title(f"{student_id}+{name}+隐写后图像灰度值直方图")
    plt.xlabel('灰度值')
    plt.ylabel('频率')

    # 设置x轴刻度为整数，间距为1
    plt.gca().xaxis.set_major_locator(plt.MultipleLocator(1))
    plt.gca().set_xticks(range(60, 81))  # 设置明确的横坐标刻度

    # 设置y轴刻度为整数，间距为1，范围为0-10
    plt.gca().yaxis.set_major_locator(plt.MultipleLocator(1))
    plt.ylim(0, 10)  # 设置y轴的范围

    # 在每个条形上方显示数值
    for patch, value in zip(patches, n):
        height = patch.get_height()
        plt.text(patch.get_x() + patch.get_width() / 2, height, f'{int(value)}',
                 ha='center', va='bottom', fontsize=10)

    plt.grid(True)
    plt.show()


def embed_message(original_image, binary_message):
    """在次低有效位嵌入秘密信息"""
    stego_image = original_image.copy()

    for i in range(len(binary_message)):
        row = i // 4
        col = i % 4
        # 将次低有效位替换为秘密信息位
        if binary_message[i] == '0':
            stego_image[row, col] = stego_image[row, col] & 0xFD  # 清除次低有效位
        else:
            stego_image[row, col] = stego_image[row, col] | 0x02  # 设置次低有效位为1

    print(f"\n本人学号为{student_id}，本人姓名为{name}，\n隐写后16个像素点的灰度值矩阵如下：")
    for row in stego_image:
        for col in row:
            print(col, end=" ")
        print()
    return stego_image


def attack_image(stego_image):
    """将所有次低位置为1"""
    attacked_image = stego_image.copy()
    attacked_image = attacked_image & ~0x02  # 将次低位全部置为0

    print(f"\n本人学号为{student_id}，本人姓名为{name}，\n隐写图像遭受攻击后图像的灰度值矩阵如下：")
    for row in attacked_image:
        for col in row:
            print(col, end=" ")
        print()
    return attacked_image


def extract_lsb(image):
    """提取次低有效位"""
    bits = []
    for row in range(4):
        for col in range(4):
            # 提取次低有效位
            slsb = (image[row, col] >> 1) & 0x01
            bits.append(str(slsb))

    result = ''.join(bits)
    print(f"\n本人学号为{student_id}，本人姓名为{name}，隐写图像遭受攻击后图像16个灰度值的次低有效位为：{result}")
    return result


def calculate_ber(original, received):
    # 确保两个序列长度相同
    if len(original) != len(received):
        raise ValueError("Original and received sequences must be of the same length.")

    # 将二进制字符串转换为整数列表
    original_bits = [int(bit) for bit in original]
    received_bits = [int(bit) for bit in received]

    # 计算错误比特数
    error_bits = sum(1 for orig, recv in zip(original_bits, received_bits) if orig != recv)

    # 计算误码率
    ber = error_bits / len(original_bits)

    print(f"攻击后次低有效位提取的秘密信息的误码率为：{ber}")

    return ber

def main():
    try:
        # 原始4x4图像灰度值
        original_image = np.array([
            [61, 64, 69, 72],
            [78, 61, 69, 72],
            [64, 64, 61, 69],
            [64, 64, 72, 61]
        ])

        # 第一步：输出ASCII和二进制
        binary_message = get_binary_message("BH")

        # 第二步：绘制原始直方图
        plot_histogram(original_image)

        # 第三步：嵌入秘密信息
        stego_image = embed_message(original_image, binary_message)

        # 第四步：绘制隐写后的直方图
        plot_histogram(stego_image, "隐写后的")

        # 第五步：攻击图像
        attacked_image = attack_image(stego_image)

        # 第六步：提取次低位
        extract_lsb(attacked_image)

        # 第七步：计算误码率
        calculate_ber("0000000000000000", "0100001001001000")

    finally:
        # 恢复终端默认颜色
        print('\033[0m')


if __name__ == "__main__":
    main()
